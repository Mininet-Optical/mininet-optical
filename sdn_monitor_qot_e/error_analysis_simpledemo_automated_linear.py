...
def get_monitors(link_dir):
    ...
    if link_dir == 'r_london-r_copenhagen/':
        ...
        #monitors=['r_hop1-r_hop2-boost', 'r_hop1-r_hop2-amp1', 'r_hop2-r_hop3-boost', 'r_hop2-r_hop3-amp1', 'r_hop3-r_hop4-boost', 'r_hop3-r_hop4-amp1', 'r_hop4-r_hop5-boost', 'r_hop4-r_hop5-amp1', 'r_hop5-r_hop6-boost', 'r_hop5-r_hop6-amp1', 'r_hop6-r_hop7-boost', 'r_hop6-r_hop7-amp1', 'r_hop7-r_hop8-boost', 'r_hop7-r_hop8-amp1', 'r_hop8-r_hop9-boost', 'r_hop8-r_hop9-amp1']
        #print(monitors)
    return monitors


def build_mon_est(links_dir,link_dict):
    _dict=link_dict.copy()
...
def build_mon_corr(links_dir,link_dict):
    # _dict = {10: {}, 30: {}, 60: {}}
...
def get_monitor_deploy(link_key, density=10, first_last='first'):
    ...
    if link_key == 'r_london-r_copenhagen':
        ...
        monitors = [stripped_name + '-amp' + str(i) for i in
                    range(1, no_monitors + 1)]
        monitors.insert(0, stripped_name + '-boost')
        monitors = monitor_deployment(monitors, density=density, first_last=first_last)
        monitors=monitor_deployment(['r_hop1-r_hop2-boost', 'r_hop1-r_hop2-amp1', 'r_hop2-r_hop3-boost', 'r_hop2-r_hop3-amp1', 'r_hop3-r_hop4-boost', 'r_hop3-r_hop4-amp1', 'r_hop4-r_hop5-boost', 'r_hop4-r_hop5-amp1', 'r_hop5-r_hop6-boost', 'r_hop5-r_hop6-amp1', 'r_hop6-r_hop7-boost', 'r_hop6-r_hop7-amp1', 'r_hop7-r_hop8-boost', 'r_hop7-r_hop8-amp1', 'r_hop8-r_hop9-boost', 'r_hop8-r_hop9-amp1'], density=density, first_last=first_last)
        return monitors
...
if tmp_bool:
    ...
    #[['london', 'brussels', 'paris', 'berlin', 'amsterdam'],['london', 'copenhagen', 'prague', 'luxembourg', 'zurich']]]
    links_dir=['r_london-r_brussels/', 'r_brussels-r_paris/', 'r_paris-r_berlin/', 'r_berlin-r_amsterdam/', 'r_london-r_copenhagen/', 'r_copenhagen-r_prague/', 'r_prague-r_luxembourg/', 'r_luxembourg-r_zurich/']
    links_dir=['r_hop1-r_hop2/', 'r_hop2-r_hop3/', 'r_hop3-r_hop4/', 'r_hop4-r_hop5/', 'r_hop5-r_hop6/', 'r_hop6-r_hop7/', 'r_hop7-r_hop8/', 'r_hop8-r_hop9/']
    densities_dir = ['density_10/', 'density_30/', 'density_60/']
    link_dict={}
    for link in links_dir:
        link_dict[link.strip('/')]={}

    mon_est = build_mon_est(links_dir,link_dict) #change automated
    ...
    # [['london', 'amsterdam'], ['london', 'paris', 'luxembourg', 'zurich']]
    # [['london', 'brussels', 'paris', 'berlin', 'amsterdam'],['london', 'copenhagen', 'prague', 'luxembourg', 'zurich']]]
    link_keys=[('r_london-r_brussels', 'r_brussels-r_paris', 'r_paris-r_berlin', 'r_berlin-r_amsterdam'), ('r_london-r_copenhagen', 'r_copenhagen-r_prague', 'r_prague-r_luxembourg', 'r_luxembourg-r_zurich')]
    link_keys=[('r_hop1-r_hop2', 'r_hop2-r_hop3', 'r_hop3-r_hop4', 'r_hop4-r_hop5', 'r_hop5-r_hop6', 'r_hop6-r_hop7', 'r_hop7-r_hop8', 'r_hop8-r_hop9')]
    ...
    mon_corr = build_mon_corr(links_dir,link_dict) #change automated
    mon_info={}
    mon_info_stat = {}
        ...
        for link_key, channels in zip(link_keys, channels_list):
            ...
            if type(link_key) is tuple:
                for i, e in enumerate(link_key, start=1):
                    ...
                    print(monitors_by_density)
                    monitor_list = []
                    for selected_monitor in monitors_by_density:
                        if (e in selected_monitor):
                            monitor_list.append(selected_monitor)
    ...
    #['r_london-r_amsterdam', ('r_london-r_paris', 'r_paris-r_luxembourg', 'r_luxembourg-r_zurich')]
    #links_dir = ['r_london-r_copenhagen/', 'r_copenhagen-r_prague/', 'r_prague-r_luxembourg/', 'r_luxembourg-r_zurich/']

    plt.rc('axes', labelsize=14)
    plt.rc('legend', fontsize=12)
    density_dict_orig={10: [], 20: [], 30: [], 40: [], 50: [], 60: [], 70: [], 80: []}
    density_dict_correct = {10: [], 20: [], 30: [], 40: [], 50: [], 60: [], 70: [], 80: []}
    for link_dir in links_dir:
        fig = plt.figure()
        ...
        for density in densities:
            ...
            density_dict_orig[density].extend(eo)
            density_dict_correct[density].extend(ec)
            ...
            plt.plot(mon_info_stat[link_dir][density],mon_point_values, color=color, marker=marker,linestyle='None', markersize=12)
            plt.legend()
            plt.grid(True)
    ...
    fig = plt.figure()
    bl = True
    print(len(density_dict_correct[10]))
    link_key1="hop1-hop9"
    for density in densities:

        if bl:
            bl = False
            label_eo = 'original error-' + link_key1
            plt.plot(density_dict_orig[density], color='g', label=label_eo)


        label_ec = 'c-' + link_key1 + '-' + str(density)
        color = colors.pop()
        marker = markers.pop()

        plt.plot(density_dict_correct[density], color=color, label=label_ec, marker=marker)
        #plt.plot(mon_info_stat[link_dir][density], mon_point_values, color=color, marker=marker, linestyle='None',
         #        markersize=12)
        plt.legend()
        plt.grid(True)
        plt.xlabel('Monitoring locations')
