    ...
    if pattern == 'sawtooth':
        if source == 'office':
            if time % 24 < timelength:
                factor = ((sp.signal.sawtooth(2 * np.pi * (time % 24) / timelength, 1) + 1) / 2) * (1-floor+.001) + floor
                return factor
            else:
                return floor
        if source == 'resident':
            if shift < (float(time) % 24) and (float(time) % 24) < (timelength + shift):
                factor = ((sp.signal.sawtooth(2 * np.pi * ((time % 24) -shift) / timelength, 1) + 1) / 2) * (1-floor+.001) + floor
                return factor
            else:
                return  floor
    if pattern == 'triangle':
        ...
        for src in RU_ROADMS:
            ...
            while factor*MAX_traf[src]/CPRI_CAP > len(ROADM_TRAF[src]):
                ...
                if len(path_last)<len(path_first) and BBU_traf['t%d' % NUM_NODE] < BBU_limit['t%d' % NUM_NODE]:
                    print('BBU R6 resource',BBU_traf['t%d' % NUM_NODE], BBU_limit['t%d' % NUM_NODE])
                    dst = 'r%d' % NUM_NODE
                    dst_t = 't%d' % NUM_NODE
                    dst_back = 'r1'
                    dst_t_back = 't1'
...
if __name__ == '__main__':
    ...
    TrafficTest(shift=0, floor=0.001, timelength=8, pattern='sawtooth', load=35000, days=7, filename='Control.txt')
