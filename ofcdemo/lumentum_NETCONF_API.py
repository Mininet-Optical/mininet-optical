...
class lumentum_NETCONF(object):
    ...
    def _ConfigWSS(self, node_ip, status, conn_id, module_id, input_port, output_port, start_freq, end_freq, attenuation, block, name):
    ...
    def _WSSMonitor(self,ev):
        node = Lumentum(str(ev.node_ip), str(USERNAME), str(PASSWORD))
        mux_info = {}
        demux_info = {}
        try:
