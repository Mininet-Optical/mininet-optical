...
def twoRoadmPhyNetwork( lengths=[50*km] ):
    ...
    r1.install_switch_rule(
        rule_id = 1, in_port=ADD_DROP_0, out_port=LINE_PORT, signal_indices=[1] )
    r1.configure_voa(channel_id = 1, output_port = LINE_PORT,operational_power_dB = 0)
    r1.install_switch_rule(
        rule_id = 2, in_port=ADD_DROP_1, out_port=LINE_PORT, signal_indices=[2])
    r1.configure_voa(2, LINE_PORT, 2)

    r2.install_switch_rule(
        rule_id = 1, in_port=LINE_PORT, out_port=TX_0, signal_indices=[1])
    r2.configure_voa(1, TX_0, 0)
    r2.install_switch_rule(
        rule_id = 2, in_port=LINE_PORT, out_port=TX_1, signal_indices=[2])
...
def twoRoadmPhyTest():
    ...
    # start test
    # launch power 0dbm
    t11.operation_power = db_to_abs(0)
    t12.operation_power = db_to_abs(0)
    t21.operation_power = db_to_abs(0)
    t22.operation_power = db_to_abs(0)
