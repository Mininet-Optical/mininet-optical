...
def run( net, N=1 ):
    ...
    net.neighbors = adjacencyDict( net.allLinks, net.roadmLinks)
    ...
    for src, dst in net.pairs:
        if (src=='t0' and dst=='t3') or (src=='t1' and dst=='t4') or (src=='t2' and dst=='t5'):
            ...
            print(dst, '->', src, path, net.pairChannels[dst, src])
...
def adjacencyDict( links, roadm_links ):
...
def configureTerminals( net, power=0.0):
    ...
    for terminal in net.terminals:
        if terminal == 't0' or terminal == 't1' or terminal == 't2':
...
def sixNodeRing_Unidirectional_Roadm_link_controller():
    net = RESTProxy()
    run( net )
